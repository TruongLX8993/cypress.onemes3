...
describe("Tiếp nhận", () => {
    ...
    it('Tiếp nhận lại bệnh nhân cũ đã HTTT', () => {
        ...
        // cy.get('li.select2-search').find('input.select2-search__field').type('LINH TEST 051504');

        const maxAttempts = 10;
        const searchText = 'LINH';

        cy.get('#drpSelectTimKiem').parent().find('span.selection span.select2-selection ul.select2-selection__rendered').click();

        for (let attempts = 0; attempts < maxAttempts; attempts++) {
            cy.get('li.select2-search').find('input.select2-search__field').clear().type(searchText);
            cy.wait(5000);
            cy.get('#select2-drpSelectTimKiem-results').then($results => {
                if ($results.find('tr').length > 0) {
                    cy.wrap($results).find('tr:first').click();
                    attempts = maxAttempts;
                } else if (attempts === maxAttempts - 1) {
                    throw new Error("Không tìm thấy kết quả sau số lần thử quá giới hạn");
                }
            });
